...
foreach y of local years { // loop through all years
	...
	centile redi_dV_hinc_shp_2017_median  if year == `y', centile(5 50 95) 
	centile redi_dV_hinc_shp_2017_median  if year == `y', centile(5 50 95) meansd 
...
egen grand_mean = mean(redi_dV_hinc_shp_2017_mean), by(year)


di in red "grand gini for year `y'" 

egen grand_gini = mean(redi_dV_hinc_shp_2017_gini), by(year)
