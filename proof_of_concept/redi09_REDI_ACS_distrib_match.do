...
di in red "Create natural log income variables for acs_hinc_cont_`conv_year'"

gen redi_hinc_cont_inf`conv_year'_ln = ln(redi_hinc_cont_inf`conv_year')
gen acs_hinc_cont_`conv_year'_ln = ln(acs_hinc_cont_`conv_year')
gen redi_hinc_cont_inf`conv_year'_log = log(redi_hinc_cont_inf`conv_year')
gen acs_hinc_cont_`conv_year'_log = log(acs_hinc_cont_`conv_year')

clonevar redi_inc = acs_hinc_cont_`conv_year'
replace  redi_inc = 1 if acs_hinc_cont_`conv_year' <= 0
...
*Log plot
/*
distplot acs_hinc_cont_`conv_year' redi_hinc_cont_inf`conv_year', ///
legend( ///
...
distplot acs_hinc_cont_`conv_year'_ln redi_hinc_cont_inf`conv_year'_ln, ///
...
save $deriv/redi09_REDI_ACS_distrib_match-`p'.dta, replace

keep year perwt redi_hinc_cont_inf`conv_year' acs_hinc_cont_`conv_year'  repwtp* 
...
rename acs_hinc_cont_`conv_year'  var_hinc1
rename redi_hinc_cont_inf`conv_year'  var_hinc2
	generate id = _n
...
foreach y of local years { // loop through all years
	...
	*ttest redi_hinc_cont_inf`conv_year' == acs_hinc_cont_`conv_year'  [pweight=perwt] , unequal unpaired
