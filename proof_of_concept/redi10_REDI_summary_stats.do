...
***--------------------------***
	...
	foreach y of local years {
		...
		epctile redi_hinc_cont_inf`conv_year', ///
...
***--------------------------***
	...
	label var redi_hinc_cont_inf`conv_year'_mean ///
	...
	*	redi_hinc_cont_inf`conv_year'_mean
	*year 2016  REDI
	...
	*egen redi_hinc_cont_inf`conv_year'_sd = ///
	*	sd(redi_hinc_cont_inf`conv_year'), by(year)

***--------------------------***
// # GINI COEFFICIENT
***--------------------------***
	capture drop redi_hinc_cont_inf`conv_year'_gini
	gen redi_hinc_cont_inf`conv_year'_gini = .
	label var redi_hinc_cont_inf`conv_year'_gini "REDI hinc shp `conv_year' Gini"
	...
	*fastgini varname [if] [in] [weight] [, bin(#) jk Level(#) nocheck]
	...
	replace redi_hinc_cont_inf`conv_year'_gini = `gini2016' if year == 2016
	replace redi_hinc_cont_inf`conv_year'_gini = `gini2017' if year == 2017
...
***--------------------------***
	...
	capture drop repwtp* perwt repwtp pers_inc ftotinc_acs repwt  ///
		...
		acs_hinc_shp_2017lb acs_hinc_shp_2017ub acs_hinc_shp_2017 redi_hinc_cont_inf2017

	gen run_n = `run'

	*duplicates drop ///
	*	redi_hinc_cont_inf`conv_year'_mean ///
	*	redi_hinc_cont_inf`conv_year'_median, force

	duplicates drop ///
		redi_hinc_cont_inf`conv_year'_gini year, force
