*! 1.0.0 Molly M. King 13 September 2021
...
program define redi
	version 16.0
	syntax inc_var year ref_data [, inc_type inflation_year ]
	...
	foreach inc_level of local `inc_var'_levels {
		...
		if "`ref_data''" == "cps" {	
			if "`inc_type'" == "family" {
				...
				use $deriv/ki11_bidee00_ASEC-hhincome.dta, clear // use ASEC household dataset
				local ref_data "$deriv/ki11_bidee00_ASEC-hhincome"
		...
		*in this artificial data set (which will later be matched to Research data set)
		...
		label var 	`redi_inc_var'_n "`inc_type' (REDI) `inc_types' count of respondents in each income bin in post-merge dataset'"
...
if "`inflation_year''" != "" { // if inflation year is not empty	
	...
	* Locals for final variables using inflation-adjustments
	local final_inc_var "redi_`inc_var_short'_`inc_types'_`conv_year'"
	local final_ub "ub_`inc_var_short'_`inc_types'_`conv_year'"
	local final_lb "lb_`inc_var_short'_`inc_types'_`conv_year'"
	...
	*original income variable, adjusted for inflation
	gen `final_inc_var' = `inc_var_name' / conv_factor
	format `final_inc_var' %6.0fc
	label var `final_inc_var' "REDI continuous inflation-adjusted `inc_var' income, from `inc_types' categories, `conv_year' dollars"
 
	gen `final_ub' = `inc_var'_ub / conv_factor
	format `final_ub' %6.0fc
	label var `final_ub' "Inflation-adjusted Upper Bound for `final_inc_var', from `inc_types' categories, `conv_year' dollars"

	gen `final_lb' = `inc_var'_lb / conv_factor
	format `final_lb' %6.0fc
	label var `final_lb' "Inflation-adjusted Lower Bound for `final_inc_var', from `inc_types' categories, `conv_year' dollars"
	...
	// FINAL VARIABLES if inflation not specified
		* Locals for final variables using inflation-adjustments
	local final_inc_var "redi_`inc_var_short'_`inc_types'"
	local final_ub "ub_`inc_var_short'_`inc_types'"
	local final_lb "lb_`inc_var_short'_`inc_types'"

	
	*original income variable
	gen `final_inc_var' = `inc_var_name'
	format `final_inc_var' %6.0fc
	label var `final_inc_var' "REDI continuous inflation-adjusted `inc_var' income, from `inc_types' categories"
 
	gen `final_ub' = `inc_var'_lb
	format `final_ub' %6.0fc
	label var `final_ub' "Upper Bound for `final_inc_var', from `inc_types' categories"

	gen `final_lb' = `inc_var'_lb
	format `final_lb' %6.0fc
	label var `final_lb' "Lower Bound for `final_inc_var', from `inc_types' categories"
